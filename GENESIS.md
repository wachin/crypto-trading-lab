...
> **If this repository already contains source code:** this is a
> project in progress, not a fresh start. Read `AGENT-HANDOFF.md`
> first (it maps the current state and the continuation point), then
> `AGENTS.md` for the ground rules, then continue from where the
> roadmap's `[x]` marks end. The instructions below still apply as
> the project's philosophy and long-term method.

> **If this repository is empty of source code** (fresh start): follow
> the Phase 0 instructions below as written.
...
### 3. Be honest about profitability — the goal is a real chance, never a promise

Crypto Trading Lab must never claim that a strategy **will** make money. That would be a lie, and lying to the user would be a betrayal of this project's purpose.

But understand the honest truth clearly, like a farmer understands the rain:

- No one can promise in which year it will rain well. A farmer who depends on rain cannot be **assured** of profits or protected from losses.
- In the same way, no program — this one included — can **guarantee** profits to whoever uses it.
- And yet, some people do make a living from the cryptocurrency markets, just as some farmers do harvest successfully season after season. They are the ones who study, measure, and protect themselves from bad years — not the ones who hope.

Therefore this project has a real, serious purpose:

- Build the best possible **research tool** to search for a genuine, measurable edge in the markets — following the discipline of the book *Advances in Financial Machine Learning* (Marcos López de Prado), already studied in `docs/en/developers/reference-projects.md`;
- try honestly to help the user obtain **some real profits** through statistical evidence, rigorous backtesting, out-of-sample validation, and strict risk control — never through hope, hype, or fabricated metrics;
- and because the developer of this project is in a vulnerable economic situation, **capital protection comes before profit seeking**: money needed to live must never be risked. The risk-of-ruin controls, the kill switch, and the mandatory qualification pipeline are not optional features — they are the safety net that makes survival possible.

The right sentence is: "This program tries, with scientific honesty, to find and validate an edge — and it tells you the truth when there isn't one."
...
**MANDATORY STOP RULE — dependencies:**

If at any point you discover that a new dependency must be installed:

1. **STOP** development immediately at that task. Do not continue.
2. Do **NOT** install anything yourself: no `apt`, no `pip`, no `pip --user`, no downloading packages or wheels.
3. **Notify the developer** with:
   - the exact package name;
   - whether it comes from the Debian repositories or from PyPI;
   - why it is needed;
   - the exact command the developer must run.
4. If the package exists in the Debian repositories, the developer installs it with `apt` (for example: `sudo apt install python3-<package>`). After the developer confirms, verify the import works and continue.
5. If the package exists **only on PyPI**, the developer must create and activate a virtual environment first:
   ```bash
   python3 -m venv .venv
   source .venv/bin/activate
   pip install <package>
   ```
   You must never create, activate, or install into a venv on your own — the developer does it, you use it afterwards.
6. So far no venv has been needed because every required package is available in the distribution. The first PyPI-only dependency will require the venv procedure above, and you must warn the developer **before** that happens.
7. Never proceed past a missing dependency, never mock a missing import to pretend work continues, and record the blocker in your report until the developer resolves it.
